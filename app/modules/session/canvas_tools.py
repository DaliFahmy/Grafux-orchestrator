    ...
    _func(
        ...
        "Create a new saved block (only after user confirmation).",
        {
            "block_type": _str_prop(
                ...
                "- openram: COMPILE AN SRAM MACRO from memory parameters (chip "
                "design). Use it whenever the user asks for a memory, an SRAM, a "
                "RAM or a register file as a hardware block to be built, and set "
                "'top' to the macro name. It takes no design input -- word_size "
                "and num_words are the request -- and it produces GDS, LEF, "
                "Liberty, a behavioural Verilog model and a SPICE netlist. Not the "
                "'memory' block, which stores DATA for the canvas and builds no "
                "hardware; and not 'code_hdl', which would write inferred RTL a "
                "synthesizer turns into flip-flops rather than a real SRAM array.\n"
                "- opengcram: COMPILE A GAIN-CELL (GCRAM / eDRAM-style 2T) MEMORY "
                "MACRO with OpenGCRAM (chip design). Use it only when the user asks "
                "for a gain-cell memory specifically; for an ordinary SRAM use "
                "openram. Same ports and outputs as openram plus 'gc_type' (OS, Si "
                "or hybrid); set 'top' to the macro name. It needs a technology "
                "archive the user supplies on its tech_archive port, because no "
                "public technology contains a gain cell.\n"
                "- analogue_simulator: SIMULATE AN ANALOGUE / TRANSISTOR-LEVEL "
                "CIRCUIT with ngspice (SPICE). Use it whenever the user wants to "
                "simulate, characterise or measure a circuit at the transistor or "
                "component level -- an inverter's delay, an amplifier's gain or "
                "bandwidth, an RC filter, a bias point, a DC sweep. Set 'pdk' "
                "(sky130A, gf180mcuD, or none for generic models) and describe the "
                "circuit in the description; the netlist is written from it at Run "
                "unless a netlist is wired in. Outputs measurements, an operating "
                "point and plottable waveforms. Not verilator, which simulates "
                "digital RTL.\n"
                ...
                "-> yosys -> openroad; openram (or opengcram) sits BESIDE that chain rather than in "
                "it, supplying the memory macro a design instantiates (its "
                "verilog_model output wires into a verilator block's rtl for "
                "simulation). An analogue_simulator's waveforms output wires straight "
                "into a plotter block's data input, and openram's spice output can "
